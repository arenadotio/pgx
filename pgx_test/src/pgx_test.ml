...
module Make_tests (IO : Pgx.IO) = struct
  ...
  let with_conn ?database f =
    try_with (fun () -> with_conn ?database (fun _ -> return ()))
    >>= function
      ...
      | Ok () -> with_conn ?database f

  let with_temp_db f =
    let random_db () =
      let random_char () =
        10 |> Random.int |> string_of_int |> fun s -> s.[0]
      in
  ...
  let assert_error_test query () =
    ...
    >>= function Ok _ -> failwith "error expected" | Error _ -> return ()
  ...
  let make_tests suite_name tests =
    deferred_list_map tests ~f:(fun ((name, test): string * async_test) ->
        try_with test
        >>| function Ok () -> (name, `Ok) | Error exn -> (name, `Exn exn) )
    >>| List.map (fun (name, res) ->
            name >:: fun _ -> match res with `Ok -> () | `Exn x -> reraise x )
    >>| ( >::: ) suite_name
  ...
  let pretty_print_string_option_list_list l =
    List.map
      (fun sol ->
        let sl =
          List.map
            (function None -> "None" | Some s -> "Some \"" ^ s ^ "\"")
            sol
        in
  ...
  let pretty_print_string_option_list_list_list l =
    List.map pretty_print_string_option_list_list l |> string_list_to_string
  ...
  let run_tests () =
    ...
    let tests =
      ...
      ; ( "query - 1 query"
        , fun () ->
            with_conn (fun dbh ->
                simple_query dbh "select 1" >>| assert_equal [[[Some "1"]]] )
        )
      ; ( "query - multiple"
        , fun () ->
            with_conn (fun dbh ->
                simple_query dbh "select 1; select 2; select 3"
                >>| assert_equal [[[Some "1"]]; [[Some "2"]]; [[Some "3"]]] )
        )
      ; ( "query - multiple single query"
        , fun () ->
            with_conn (fun dbh ->
                simple_query dbh
      ...
      ; ( "query - empty"
        , fun () ->
            with_conn (fun dbh -> simple_query dbh "" >>| assert_equal []) )
            ...
            Prepared.(
              with_prepare dbh ~query:"values (1,2),(3,4)" ~f:(fun s ->
                  execute_fold s ~params:[] ~init:[] ~f:(fun acc a ->
                      return (a :: acc) ) ))
            >>| assert_equal [[Some "3"; Some "4"]; [Some "1"; Some "2"]] )
      ; ( "test execute_prepared"
        , fun () ->
            with_conn
            @@ fun dbh ->
            Prepared.(
              prepare dbh ~query:"values (1,2),(3,4)" >>= execute ~params:[])
            >>| assert_equal [[Some "1"; Some "2"]; [Some "3"; Some "4"]] )
      ; ( "test execute_iter"
        , fun () ->
            let n = ref 0 in
      ...
      ; ( "in_transaction invariant"
        , fun () ->
            ...
            >>| function
              ...
              | Error exn -> reraise exn )
      ; ( "triple prepare no infinite loop"
        , fun () ->
            with_conn
            @@ fun dbh ->
      ...
      ; ( "query with SET"
        , fun () ->
            with_conn (fun dbh ->
                simple_query dbh
                  "SET LOCAL TIME ZONE 'Europe/Rome'; SELECT 'x'"
                >>| function
                  | [[]; [[res]]] ->
                      Pgx.Value.to_string_exn res
                      |> assert_equal ~printer:(fun x -> x) "x"
                  | _ -> assert false ) )
      ; ("ping", fun () -> with_conn (fun dbh -> ping dbh))
      ; ( "with_prepare and describe_statement"
        , fun () ->
            with_conn
            @@ fun dbh ->
      ...
      ; ( "should fail without sequencer"
        , fun () ->
            with_conn (fun dbh ->
                ...
                >>| fun _ -> () ) )
      ; ( "copy out simple query"
        , fun () ->
            with_temp_db (fun dbh ~db_name:_ ->
                simple_query dbh
                  "CREATE TABLE tennis_greats ( name            varchar(40), \
                   grand_slams     integer); INSERT INTO tennis_greats VALUES \
                   ('Roger Federer', 19), ('Rafael Nadal', 15); COPY \
                   tennis_greats TO STDOUT (DELIMITER '|')"
                >>| assert_equal
                      [ []
                      ; []
                      ; [ [Some "Roger Federer|19\n"]
                        ; [Some "Rafael Nadal|15\n"] ] ] ) )
      ; ( "copy out extended query"
        , fun () ->
            with_temp_db (fun dbh ~db_name:_ ->
                execute dbh
                  "CREATE TABLE tennis_greats ( name            varchar(40), \
                ...
                >>= fun _ ->
                execute dbh
                  "INSERT INTO tennis_greats VALUES ('Roger Federer', 19), \
                   ('Rafael Nadal', 15);"
                >>= fun _ ->
                execute dbh "COPY tennis_greats TO STDOUT (DELIMITER '|')" )
            >>| assert_equal
                  [[Some "Roger Federer|19\n"]; [Some "Rafael Nadal|15\n"]] )
      ; ( "execute_prepared_iter and transact test"
        , fun () ->
            with_temp_db (fun dbh ~db_name:_ ->
                with_transaction dbh (fun dbh ->
                    execute dbh
                      "CREATE TABLE tennis_greats ( name            \
                       varchar(40), grand_slams     integer);"
                    >>= fun _ ->
                    execute dbh
                      "INSERT INTO tennis_greats VALUES ('Roger Federer', \
                       19), ('Rafael Nadal', 15);"
                    >>= fun _ ->
                    let open Prepared in
                    with_prepare dbh
                      ~query:
                        "SELECT * FROM tennis_greats WHERE name=$1 AND \
                         grand_slams=$2" ~f:(fun s ->
                        let acc = ref [] in
                        execute_iter s
                          ~params:
                            Pgx.Value.[of_string "Roger Federer"; of_int 19]
                          ~f:(fun fields -> return (acc := fields :: !acc))
                        >>= fun () -> return !acc ) )
                >>| assert_equal [[Some "Roger Federer"; Some "19"]] ) )
      ; ( "commit while not in transaction"
        , fun () ->
            try_with (fun () ->
                with_conn
                @@ fun dbh ->
                begin_work dbh
                >>= fun dbh -> commit dbh >>= fun () -> commit dbh )
            >>= function
              | Ok _ ->
                  failwith "commit while not in transaction error expected"
              | Error _ -> return () )
      ; ( "rollback while not in transaction"
        , fun () ->
            try_with (fun () ->
                with_conn
                @@ fun dbh ->
                begin_work dbh
                >>= fun dbh -> commit dbh >>= fun () -> rollback dbh )
            >>= function
              | Ok _ ->
                  failwith "rollback while not in transaction error expected"
              | Error _ -> return () )
      ; ( "alive test"
        , fun () -> with_conn @@ fun dbh -> alive dbh >>| assert_equal true )
            ...
            with_temp_db (fun dbh ~db_name:_ ->
                execute dbh
                  "CREATE TABLE tennis_greats ( name            varchar(40), \
                   grand_slams     integer);"
                >>= fun _ ->
                with_transaction ~isolation:Pgx.Isolation.Serializable dbh
                  (fun dbh ->
                    execute dbh
                      "INSERT INTO tennis_greats VALUES ('Roger Federer', 19);"
                )
                >>= fun _ ->
                with_transaction ~isolation:Pgx.Isolation.Repeatable_read dbh
                  (fun dbh ->
                    execute dbh
                      "INSERT INTO tennis_greats VALUES ('Rafael Nadal', 15);"
                )
                >>= fun _ ->
                with_transaction ~isolation:Pgx.Isolation.Read_committed dbh
                  (fun dbh ->
                    execute dbh
                      "INSERT INTO tennis_greats VALUES ('Novak Djokovic', 12);"
                )
                >>= fun _ ->
                with_transaction ~isolation:Pgx.Isolation.Read_uncommitted dbh
                  (fun dbh ->
                    execute dbh
                      "INSERT INTO tennis_greats VALUES ('Andy Murray', 3);" )
                >>= fun _ ->
                let open Prepared in
                with_prepare dbh
                  ~query:
                    "SELECT * FROM tennis_greats WHERE name=$1 AND \
                     grand_slams=$2" ~f:(fun s ->
                    let acc = ref [] in
                    execute_iter s ~params:[Some "Andy Murray"; Some "3"] ~f:
                      (fun fields -> return (acc := fields :: !acc) )
                    >>= fun () -> return !acc )
      ...
      ; ( "multi typed table"
        , fun () ->
            with_temp_db (fun dbh ~db_name:_ ->
                simple_query dbh
                  "CREATE TABLE multi_typed(uuid uuid, int int, string text, \
                   numeric numeric);"
                >>= fun _ ->
                ...
                in
                execute dbh ~params
                  "INSERT INTO multi_typed (uuid, int, string, numeric) \
                   VALUES ($1, $2, $3, $4)"
                >>= fun _ ->
                ...
                >>| function
                  | [[[uuid; int_; string_; numeric]]] ->
                      ...
                      assert_equal (Some all_chars)
                        ~printer:(function Some v -> v | None -> "(None)")
                        string_ ;
                      assert_equal (Some "9223372036854775807") numeric
                  | _ ->
                      failwith
                        "Error: multi typed table: got unexpected query result"
            ) )
               ...
               back out *)
                ...
                |> deferred_list_map ~f:(fun (query, param, expect) ->
                       ...
                       >>| function
                         | [[Some actual]] ->
                             assert_equal
                               ~printer:(fun x -> Printf.sprintf "'%s'" x)
                               expect actual
                         | _ -> assert false ) )
            >>| List.iter (fun () -> ()) ) ]
    in
    make_tests "pgx_async" tests >>| run_test_tt_main ~exit
end
