...
let pp_inet ppf (addr, port) =
  Format.fprintf ppf "%a:%d" Ipaddr.pp addr port

let equal_inet (a1, p1) (a2, p2) =
  Ipaddr.compare a1 a2 = 0 && p1 = p2
...
let make_test name typ to_value of_value of_value_exn values
      fail_values =
  let fail_tests =
    (Alcotest.test_case "null required input" `Quick (fun () ->
       Alcotest.check_raises "non-null conversion"
    ...
    List.map (fun str ->
      ...
      Alcotest.test_case test_name `Quick @@ fun () ->
        ...
        Alcotest.check_raises "conversion error"
          (Conversion_failure msg) (fun () -> ignore (of_value value)))
      fail_values
  in
  let success_opt_tests =
    None :: List.map (fun v -> Some v) values
    |> List.map (fun expect ->
      let test_name = Format.asprintf "good conversion - %a"
                        Alcotest.(pp (option typ)) expect in
      Alcotest.test_case test_name `Quick @@ fun () ->
...
let () =
  ...
  Alcotest.run "Pgx.Value"
  [ make_test "bool" Alcotest.bool
      of_bool to_bool to_bool_exn
      [ true
      ; false ]
      [ "" ; "asd" ]
  ; make_test "float" Alcotest_ext.our_float
      ...
      ; min_float ]
      [ "" ; "asd" ]
  ; make_test "hstore" Alcotest_ext.hstore
      ...
      [ "asd" ; "=>" ; "a=>" ; "=>v" ]
  ; make_test "inet" Alcotest_ext.inet
  ...
  ; make_test "int" Alcotest.int
      ...
      ; min_int ]
      [ "" ; "asd" ; "t" ; "f" ]
  ; make_test "int32" Alcotest.int32
      ...
      Int32.([ zero
             ...
             ; min_int ])
      [ "" ; "asd" ; "t" ; "f" ]
  ; make_test "int64" Alcotest.int64
      ...
      Int64.([ zero
             ...
             ; min_int ])
      [ "" ; "asd" ; "t" ; "f" ]
  ; make_test "list" Alcotest.(list Alcotest_ext.value)
      ...
      ; [ of_bool true
        ...
        ; of_string all_chars ] ]
      [ "" ; "asd" ]
  ; make_test "point" Alcotest.(Alcotest_ext.(pair our_float our_float))
      ...
      ; -5., max_float ]
      [ "" ; "asd" ; "5." ]
  ; make_test "string" Alcotest.string
      of_string to_string to_string_exn
      [ ""
      ; "this is a test string"
      ; all_chars ]
      []
  ; make_test "unit" Alcotest.unit
      (fun () -> unit) to_unit to_unit_exn
      [ () ]
      [ "asd" ]
  ; make_test "uuid" Alcotest_ext.uuid
