...
module Isolation = struct
  type t = Serializable | Repeatable_read | Read_committed | Read_uncommitted
  [@@deriving sexp]
...
module Ready = struct
  type t = Idle | In_transaction | Error | Other of char [@@deriving sexp]
...
module Error_response = struct
  ...
  let should_print t ~verbose =
    if verbose < 1 then false
      ...
      match t.severity with "ERROR" | "FATAL" | "PANIC" -> true | _ -> false
    else true
  ...
  let to_string ?(verbose= false) t =
    ...
    let field_info =
      if verbose then
        List.map
          (fun (field_type, field) -> sprintf "%c: %s" field_type field)
          t.custom
      else []
    in
    String.concat "\n" (msg :: field_info)
  ...
  let%test_module "to_string and should_print inline tests" =
    ( module struct
      ...
      let%test_unit "to_string tests: print msg when verbose = false" =
        ...
        [%test_result : string] ~expect:"INFO: 5: test"
      ...
      let%test_unit "should_print tests: print if verbose > 1 no matter \
                     t.severity" =
        let verbose = 2 in
...
module Message_in = struct
  type copy_format = Text | Binary [@@deriving sexp]
  ...
  let read (typ, msg) =
    ...
    let get_char where =
      if !pos < len then (
        ...
        fail_parse ("Pgx: parse_backend_message: " ^ where ^ ": short message")
    in
    ...
    let get_string () =
      ...
      let rec loop () =
        ...
        if c <> '\000' then ( Buffer.add_char buf c ; loop () )
    ...
    let get_many f =
      ...
      let fields = ref [] in
      for _ = 0 to num - 1 do fields := f () :: !fields done ;
      List.rev !fields
    in
    let msg =
      match typ with
      ...
      | 'H' ->
          let format_code_to_format = function
            ...
            | 1 -> Binary
            | format_code ->
                fail_msg "Unused CopyOutResponse format: %d" format_code
          in
          let format_ = format_code_to_format (get_int8 ()) in
          let formats =
            get_many (fun () -> format_code_to_format (get_int16 ()))
          in
...
module Message_out = struct
  type prepare = {name: string; query: string; types: oid list}
  [@@deriving sexp]
  ...
  type bind = {portal: string; name: string; params: string option list}
  [@@deriving sexp]

  type startup = {user: string; database: string} [@@deriving sexp]
  ...
  type t =
    ...
    | Bind of bind
    (* B *)
    | Close_statement of statement
    (* CP *)
    | Close_portal of portal
    (* CS *)
    | Describe_statement of statement
    (* DS *)
    | Describe_portal of portal
    (* DP *)
  ...
  let add_string_no_trailing_nil buf str =
    check_str str ; Buffer.add_string buf str

  let add_string msg str =
    add_string_no_trailing_nil msg str ;
    add_byte msg 0

  let str s = check_str s ; s ^ "\000"
...
let deserialize_hex str =
  ...
  while !i < len do
    ...
    i := !i + 2 ;
    if is_hex_digit hi_nibble && is_hex_digit lo_nibble then
...
let deserialize_string_escape str =
  ...
  while !i < len do
    ...
    if c = '\\' then (
      incr i ;
      if !i < len && str.[!i] = '\\' then ( Buffer.add_char buf '\\' ; incr i )
      else if
        !i + 2 < len
...
let deserialize_string str =
  if String.starts_with str "\\x" then deserialize_hex str
  else deserialize_string_escape str

module Value = Pgx_value

type param = Value.t [@@deriving sexp_of]

type result = Value.t [@@deriving sexp_of]

type row = Value.t list [@@deriving sexp_of]
...
module type S = sig
  ...
  module Prepared : sig
    ...
    val prepare :
      ?name:string -> ?types:oid list -> t -> query:string -> s monad
    ...
    val describe_portal :
      ?portal:string -> s -> Result_desc.t list option monad
  end
  ...
  val execute_iter :
    ?params:param list -> t -> string -> f:(row -> unit monad) -> unit monad

  val execute_many :
    t -> query:string -> params:param list list -> row list list monad
...
module Make (Thread : IO) = struct
  ...
  type conn =
    ...
    (* Used to generate statement names *) }
  [@@deriving sexp]
  ...
  let send_message {chan; _} msg =
    ...
    *)
    ...
    ( if debug_protocol then
        Thread.debug
          (sprintf "> %s%d %S"
             (match typ with None -> "" | Some c -> sprintf "%c " c)
             len msg)
    else return () )
    >>= fun () ->
    (* Write the type byte? *)
    (match typ with None -> Thread.return () | Some c -> output_char chan c)
    >>= fun () ->
  ...
  *)
  ...
  let send_recv conn msg =
    send_message conn msg >>= fun () -> receive_message conn

  (* Handle an ErrorResponse anywhere, by printing and raising an exception. *)
  let pg_error ?(sync= false) ~conn fields =
    ( if Error_response.should_print fields ~verbose:conn.verbose then
        Thread.debug
          (Error_response.to_string ~verbose:(conn.verbose > 1) fields)
    else return () )
    >>= fun () ->
    ...
    let rec loop () =
      ...
      >>= function Message_in.ReadyForQuery _ -> return () | _ -> loop ()
    in
  ...
  let connect ?host ?port ?user ?password ?database
      ?(unix_domain_socket_dir= "/tmp") ?verbose
      ?(max_message_length= Sys.max_string_length) () =
    (* Get the username. *)
    ( match user with
    | Some user -> return user
    | None ->
      try return (Sys.getenv "PGUSER") with Not_found -> Thread.getlogin () )
    >>= fun user ->
    (* Get the password. *)
    let password =
      match password with
      | Some password -> password
      | None -> try Sys.getenv "PGPASSWORD" with Not_found -> ""
    in
    (* Get the database name. *)
    let database =
      match database with
      | Some database -> database
      | None -> try Sys.getenv "PGDATABASE" with Not_found -> user
    in
    ...
    let sockaddr =
      ...
      | None ->
        try Inet (Sys.getenv "PGHOST", port) with Not_found ->
    ...
    let verbose =
      ...
      | None ->
        try Sys.getenv "PGD_PGX_VERBOSE" |> int_of_string with Not_found -> 0
    in
    let id = next_id () in
      ...
      >>= function
        ...
        *)
        ...
        | Message_in.AuthenticationMD5Password salt ->
            ...
            let digest =
              password ^ user |> Digest.string |> Digest.to_hex
              |> (fun x -> x ^ salt)
  ...
  module Prepared = struct
    type s = {conn: conn Sequencer.t sexp_opaque; name: string}
    [@@deriving sexp_of]
    ...
    let execute_iter ?(portal= "") {name; conn} ~params ~f =
      ...
      let params =
        List.map
          (fun s -> Value.to_string s |> Option.map encode_unprintable)
          params
      in
      Sequencer.enqueue conn (fun conn ->
          send_message conn
          ...
          let rec loop () =
            ...
            >>= function
              ...
              | Message_in.DataRow fields ->
                  List.map
                    (Option.bind (fun v ->
                         deserialize_string v |> Value.of_string ))
                    fields
                  |> f >>= loop
              ...
              *)
              ...
              | Message_in.CopyOutResponse (format_, format_list) -> (
                ...
                | Message_in.Text ->
                    List.iter
                      (function
                          | Message_in.Binary ->
                              fail_msg
                                "Pgx.query: Binary column found in text \
                                 CopyOutResponse"
                          | _ -> ())
                      format_list ;
                    loop ()
                | Message_in.Binary ->
                    fail_msg
                      "Pgx.iter_execute: CopyOutResponse for binary is not \
                       implemented yet" )
    ...
    let execute ?portal s ~params =
      execute_fold s ?portal ~params ~init:[] ~f:(fun acc fields ->
          return (fields :: acc) )
      >>| List.rev
    ...
    let execute_many s ~params =
      List.fold_left
        (fun acc params ->
          acc
          >>= fun acc -> execute s ~params >>| fun results -> results :: acc )
        (return []) params
      >>| List.rev
  ...
  let simple_query' dbh query =
    ...
    let rec loop acc rows state =
        ...
        | Message_in.Text ->
            List.iter
              (function
                  | Message_in.Binary ->
                      fail_msg
                        "Pgx.query: Binary column found in text CopyOutResponse"
                  | _ -> ())
              format_list ;
            loop acc rows state
        | Message_in.Binary ->
            fail_msg
              "Pgx.query: CopyOutResponse for binary is not implemented yet" )
      | _, Message_in.CopyData row ->
          loop acc
            ([row |> deserialize_string |> Value.of_string] :: rows)
            state
      ...
      | _, msg ->
          fail_msg "Pgx.query: unknown response message: %s"
            (Message_in.to_string msg)
    in
    loop [] [] `Row_desc

  let simple_query seq query =
    Sequencer.enqueue seq (fun dbh -> simple_query' dbh query)

  let execute ?(params= []) db query =
    Prepared.(with_prepare db ~query ~f:(fun s -> execute s ~params))
  ...
  let execute_fold ?(params= []) db query ~init ~f =
    ...
    with_prepare db ~query ~f:(fun s -> execute_fold s ~params ~init ~f)

  let begin_work ?isolation ?access ?deferrable seq =
    Sequencer.enqueue seq (fun conn ->
        if conn.in_transaction then
          invalid_arg
            "begin_work: cannot transact while in another transaction"
        ...
        in
        let access_str =
          match access with None -> "" | Some x -> " " ^ Access.to_string x
        in
        let deferrable_str =
        ...
        in
        let query =
          "begin work" ^ isolation_str ^ access_str ^ deferrable_str
        in
