...
let of_bool = function true -> Some "t" | false -> Some "f"
...
let to_float' t =
  ...
  | "nan" -> nan
  | _ -> try float_of_string t with Failure _ -> convert_failure "float" t
...
let of_hstore hstore =
  ...
  let string_of_mapping (key, value) =
    ...
    and value_str =
      match value with Some v -> string_of_quoted v | None -> "NULL"
    in
    key_str ^ "=>" ^ value_str
  in
...
let to_hstore' str =
  let expect target stream =
    if List.exists (fun c -> c <> Stream.next stream) target then
      convert_failure "hstore" str
  in
  ...
  let parse_main stream =
    let rec loop accum stream =
      ...
      | None -> mapping :: accum
    in
    match Stream.peek stream with Some _ -> loop [] stream | None -> []
  in
...
let of_inet (addr, mask) =
  let hostmask =
    if Unix.domain_of_sockaddr (Unix.ADDR_INET (addr, 1)) = Unix.PF_INET6 then
      128
    else 32
  in
  let addr = Unix.string_of_inet_addr addr in
  if mask = hostmask then Some addr
  else if mask >= 0 && mask < hostmask then
    Some (addr ^ "/" ^ string_of_int mask)
...
let to_inet' =
  let re =
    let open Re in
    [ group
        ( [rep (compl [set ":./"]); group (set ":."); rep1 (compl [char '/'])]
        |> seq )
  ...
  fun str ->
    try
      ...
      if mask = "" then (addr, if Re.get subs 2 = "." then 32 else 128)
...
let to_int' t = try int_of_string t with Failure _ -> convert_failure "int" t
...
let to_int32' t =
  try Int32.of_string t with Failure _ -> convert_failure "int32" t
...
let to_int64' t =
  try Int64.of_string t with Failure _ -> convert_failure "int64" t
...
let to_list' str =
  ...
  let loop (accum, quoted, escaped) = function
    ...
    | x -> Buffer.add_char buf x ; (accum, quoted, false)
  in
...
let to_point' =
  let point_re =
    ...
    [rep space; char '('; part; char ','; part; char ')'; rep space] |> seq
...
let to_unit' = function "" -> () | t -> convert_failure "unit" t
...
let to_uuid' t =
  match Uuidm.of_string t with Some u -> u | None -> convert_failure "uuid" t
