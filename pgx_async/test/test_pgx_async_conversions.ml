...
let time_roundtrip str =
  Value.to_time_exn (Some str)
...
let time_testable =
  Alcotest.testable
    (fun ppf t ->
      Format.pp_print_string ppf (printer t))
    Time.equal
...
let test_time_of_string _ =
  let expected =
    Time.of_string "2016-03-15 19:55:18.123456-04:00" in
...
let test_time_conversion_roundtrip _ =
  ...
  check_string "parse-print"
    expected_str (time_roundtrip expected_str |> printer);
  let expected_time = Time.of_string expected_str in
  check_time "print-parse"
    expected_time (Value.of_time expected_time |> Value.to_time_exn)

let time_tests =
  [ Alcotest.test_case "test time_of_string" `Quick
      test_time_of_string
  ; Alcotest.test_case "test time_of_string no milliseconds" `Quick
      test_time_of_string_no_ms
  ; Alcotest.test_case "test time conversion roundtrip" `Quick
