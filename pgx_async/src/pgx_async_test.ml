...
let ignore_empty = function [] -> () | _ :: _ -> invalid_arg "ignore_empty"

let drop_db dbh ~db_name =
  Pga.execute dbh ("DROP DATABASE " ^ db_name) >>| ignore_empty
...
let with_temp_db f =
  ...
  Pga.with_conn ~database:default_database (fun dbh ->
      ...
      Monitor.protect
        (fun () ->
          Pga.with_conn ~database:db_name (fun test_dbh -> f test_dbh ~db_name)
          )
