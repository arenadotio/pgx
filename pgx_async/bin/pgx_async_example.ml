...
let main () =
  Pgx_async_test.with_temp_db (fun dbh ~db_name:_ ->
      (* Create the table *)
      print_endline "creating table" ;
      Pgx_async.simple_query dbh
        "CREATE TABLE Company( id INT PRIMARY KEY     NOT NULL, \
         name           TEXT    NOT NULL, age            INT     NOT NULL, \
         address        VARCHAR(50), salary         REAL)"
      (* Inserting one row with prepare and execute_prepared *)
      >>= fun _ ->
      print_endline "inserting 1 row" ;
      let insert_query =
        "INSERT INTO Company (id, name, age, address, salary) VALUES ($1, $2, \
         $3, $4, $5)"
      in
      let params =
        [Some "2"; Some "John"; Some "21"; Some "Pittsburgh"; Some "100000.0"]
      in
      Pgx_async.Prepared.(
        with_prepare dbh ~query:insert_query ~f:(execute ~params))
      ...
      let params =
        [ [ Some "4"
          ...
          ; Some "Pittsburgh"
          ; Some "100000.0" ]
        ; [Some "3"; Some "John"; Some "23"; Some "New York"; Some "30000.0"]
        ]
      in
      ...
      Monitor.try_with (fun () ->
          Pgx_async.with_transaction dbh (fun dbh ->
              (* Deleting id = 2 *)
      ...
      >>= fun _ ->
      print_endline
        "selecting multiple lines at same time using execute_many. Make sure \
         order is correct!" ;
      let params = [[Some "John"; Some "50000"]; [Some "Alex"; Some "0"]] in
      Pgx_async.execute_many dbh
        ~query:"SELECT * FROM Company WHERE name = $1 and salary >= $2" ~params
      >>| fun x -> printf !"%{sexp: Pgx.row list list}\n" x )
